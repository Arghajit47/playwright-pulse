...
def generate_pie_chart(data, chart_width=300, chart_height=300):
    ...
    layout_json = json.dumps({
        ...
        "legend": {"orientation": "h", "x": 0.5, "xanchor": "center", "y": -0.08, "font": {"size": 12}},
        "showlegend": True
    })
...
def generate_spec_duration_chart(results):
    ...
    durations = [spec_durations[cat] for cat in categories]
    custom_data = [[cat, format_duration(spec_durations[cat])] for cat in categories]
    trace = {
        ...
        "marker": {"color": "#ff9800", "size": 7, "symbol": "circle"},
        "name": "Duration",
        "hovertemplate": "<b>File: %{customdata[0]}</b><br>Duration: %{customdata[1]}<extra></extra>"
    }
            ...
            if (typeof Plotly !== 'undefined') {{
                try {{
                    ...
                    const layout = {{
                        ...
                        xaxis: {{showticklabels: false, gridcolor: 'rgba(128,128,128,0.15)'}},
                        yaxis: {{
                            title: {{text: 'Total Duration (ms)'}},
                            rangemode: 'tozero',
                            tickfont: {{size: 12}},
                            gridcolor: 'rgba(128,128,128,0.15)'
                        }},
...
def generate_describe_duration_chart(results):
    ...
    COLORS = ['#9333ea','#6366f1','#0ea5e9','#10b981','#84cc16','#eab308',
              '#f97316','#ef4444','#ec4899','#8b5cf6','#06b6d4','#14b8a6',
              '#a3e635','#fbbf24','#fb923c','#f87171']
    bar_colors = [COLORS[i % len(COLORS)] for i in range(len(data))]
    custom_data = [[d['name'], d['custom']['fileName'], format_duration(d['y'])] for d in data]
    trace = {
        ...
        "name": "Duration",
        "hovertemplate": "<b>%{customdata[0]}</b><br>File: %{customdata[1]}<br>Duration: %{customdata[2]}<extra></extra>"
    }
...
# --- Email Report Logic (Ported from generate-email-report.py) ---
...
def generate_static_html(json_path, html_path):
    """Wrapper for CLI to generate a static report with trend data."""
    _generate_full_report(json_path, html_path)

def generate_dynamic_html(json_path, html_path):
    """Wrapper for CLI to generate a dynamic report."""
    _generate_full_report(json_path, html_path)

def _generate_full_report(json_path, html_path):
    import json
    from pathlib import Path
    from datetime import datetime
        ...
        for hist_run in historical_runs:
            if hist_run.get('run'):
                ...
                if flaky_val is None:
                    flaky_val = sum(1 for r in hist_run.get('results', []) if r.get('status') == 'flaky')
...
def generate_email_html(json_path):
    ...
    return generate_minified_html(report_data)
