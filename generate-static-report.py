...
def generate_test_trends_chart(trend_data):
    ...
    series = [
        {"name": "Total", "data": [r.get('totalTests', 0) for r in runs], "color": "var(--primary-color)", "marker": { "symbol": "circle" }},
        {"name": "Passed", "data": [r.get('passed', 0) for r in runs], "color": "var(--success-color)", "marker": { "symbol": "circle" }},
        {"name": "Failed", "data": [r.get('failed', 0) for r in runs], "color": "var(--danger-color)", "marker": { "symbol": "circle" }},
        {"name": "Skipped", "data": [r.get('skipped', 0) for r in runs], "color": "var(--warning-color)", "marker": { "symbol": "circle" }},
        {"name": "Flaky", "data": [r.get('flaky', 0) for r in runs], "color": "#00ccd3", "marker": { "symbol": "circle" }}
    ]
    
    runs_for_tooltip = [{"runId": r.get('runId'), "timestamp": r.get('timestamp'), "duration": r.get('duration')} for r in runs]
...
def generate_duration_trend_chart(trend_data):
    ...
    categories_string = json.dumps([f"Run {i+1}" for i in range(len(runs))])
    runs_for_tooltip = [{"runId": r.get('runId'), "timestamp": r.get('timestamp'), "duration": r.get('duration'), "totalTests": r.get('totalTests')} for r in runs]
    runs_for_tooltip_string = json.dumps(runs_for_tooltip)
...
def generate_test_history_chart(history):
    ...
    for run in valid_history:
        ...
        if status_lower == "passed": color = "var(--success-color)"
        elif status_lower == "failed": color = "var(--danger-color)"
        elif status_lower == "skipped": color = "var(--warning-color)"
        elif status_lower == "flaky": color = "var(--neutral-500)"
        else: color = "var(--dark-gray-color)"
            
        series_data_points.append({
            "y": run.get('duration', 0),
            "marker": { "fillColor": color, "symbol": "circle", "radius": 3.5, "states": { "hover": { "radius": 5 } } },
...
def generate_pie_chart(data, chart_width=300, chart_height=300):
    ...
    for d in data:
        if d.get('value', 0) > 0:
            label = d.get('label')
            if label == "Passed": color = "var(--success-color)"
            elif label == "Failed": color = "var(--danger-color)"
            elif label == "Flaky": color = "#00ccd3"
            elif label == "Skipped": color = "var(--warning-color)"
            else: color = "#CCCCCC"
            series_data_points.append({"name": label, "y": d.get('value'), "color": color})
  ...
  {{
      chart: {{
          type: 'pie', width: null, height: {chart_height - 40}, backgroundColor: 'transparent', plotShadow: false, spacingBottom: 40
      }},
      title: {{
          text: '{passed_percentage}%', align: 'center', verticalAlign: 'middle', y: 5, 
          style: {{ fontSize: '{center_title_font_size}', fontWeight: 'bold', color: 'var(--primary-color)' }}
      }},
      subtitle: {{
          text: 'Passed', align: 'center', verticalAlign: 'middle', y: 25, 
          style: {{ fontSize: '{center_subtitle_font_size}', color: 'var(--text-color-secondary)' }}
      }},
      tooltip: {{
          pointFormat: '{{series.name}}: <b>{{point.percentage:.1f}}%</b> ({{point.y}})',
          backgroundColor: 'rgba(10,10,10,0.92)', borderColor: 'rgba(10,10,10,0.92)', style: {{ color: '#f5f5f5' }}
      }},
  ...
  """
    ...
    return f"""
      <div class="pie-chart-wrapper" style="align-items: center; max-height: 450px">
          ...
          <div id="{chart_id}" style="width: 100%; max-width: {chart_width}px; height: {chart_height - 40}px;"></div>
          <script>
              document.addEventListener('DOMContentLoaded', function() {{
                  if (typeof Highcharts !== 'undefined') {{
                      ...
                      document.getElementById('{chart_id}').innerHTML = '<div class="no-data">Charting library not available.</div>';
                  }}
              }});
          </script>
      </div>
...
def generate_worker_distribution_chart(results):
    ...
    full_worker_data = [{"id": id, "name": f"Worker {id}", "tests": worker_data[id]['tests']} for id in worker_ids]
    ...
    return f"""
<style>
      .worker-modal-overlay {{ position: fixed; z-index: 1050; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.85); display: none; align-items: center; justify-content: center; backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); }}
        ...
        if (typeof Highcharts !== 'undefined') {{
          try {{
            ...
            const chartOptions = {{
              ...
              title: {{ text: null }},
              xAxis: {{ categories: {categories_string}, title: {{ text: 'Worker ID' }}, labels: {{ style: {{ color: 'var(--text-color-secondary)' }} }} }},
              ...
              plotOptions: {{ series: {{ stacking: 'normal', cursor: 'pointer', point: {{ events: {{ click: function () {{ const workerData = fullData[this.x]; window.{modal_js_namespace}.open(workerData); }} }} }} }} }},
...
def generate_ai_failure_analyzer_tab(results):
    ...
    if len(failed_tests) == 0:
        return '<div class="no-data">Congratulations! No failed tests in this run.</div>'

    total_duration = sum(t.get('duration', 0) for t in failed_tests)
    browsers_count = len(set(t.get('browser') for t in failed_tests))

    cards_html = ""
    for test in failed_tests:
        test_title = test.get('name', '').split(" > ")[-1] if test.get('name') else "Unnamed Test"
        test_json = base64.b64encode(json.dumps(test).encode('utf-8')).decode('utf-8')
        
        error_msg = test.get('errorMessage', 'No error message')
        truncated_error = error_msg[:150] + ("..." if len(error_msg) > 150 else "")

        cards_html += f"""
        <div class="compact-failure-item">
            <div class="failure-header">
                <div class="failure-main-info">
                    <h3 class="failure-title" title="{sanitize_html(test.get('name'))}">{sanitize_html(test_title)}</h3>
                    <div class="failure-meta">
                        <span class="browser-indicator">{sanitize_html(test.get('browser', 'unknown'))}</span>
                        <span class="duration-indicator">{format_duration(test.get('duration'))}</span>
                    </div>
                </div>
                <div class="ai-buttons-group">
                    <button class="compact-ai-btn" onclick="getAIFix(this)" data-test-json="{test_json}">
                        <span class="ai-text">AI Fix</span>
                    </button>
                    <button class="copy-prompt-btn" onclick="copyAIPrompt(this)" data-test-json="{test_json}" title="Copy AI Prompt">
                        <span class="copy-prompt-text">Copy AI Prompt</span>
                    </button>
                </div>
            </div>
            <div class="failure-error-preview">
                <div class="error-snippet">{format_playwright_error(truncated_error)}</div>
            ...
            <div class="full-error-details" style="display: none;">
                <div class="full-error-content">
                    {format_playwright_error(error_msg)}
                </div>
            </div>
    ...
    <div class="ai-analyzer-stats">
        <div class="stat-item">
            <span class="stat-number">{len(failed_tests)}</span>
            <span class="stat-label">Failed Tests</span>
        </div>
        <div class="stat-item">
            <span class="stat-number">{browsers_count}</span>
            <span class="stat-label">Browsers</span>
        </div>
        <div class="stat-item">
            <span class="stat-number">{round(total_duration / 1000)}s</span>
...
def generate_describe_duration_chart(results):
    ...
    for test in results:
        ...
        if describe_name:
            if describe_name.strip().lower() == "n/a" or describe_name.strip() == "":
                continue
    ...
    function getAIFix(button) {{
        ...
        try {{
            ...
            .then(text => {{
                ...
                }} catch (e) {{Sorry, something went wrong. Please try your request again.}}
.stat-number {{ 
          display: block; 
          font-size: 2em; 
          font-weight: 700; 
...
def main():
    ...
    # Step 1: Execute generate-trend.mjs
    archive_args = ["--outputDir", custom_output_dir] if custom_output_dir else []
    try:
        subprocess.run(["node", str(archive_run_script_path)] + archive_args, check=True)
