...
- **✨ New Features**:
  ...
  - **Custom Logo**: Added a new optional `logo` string option in the reporter config. Users can provide a local image path to override the default Playwright Pulse logo, favicon, in all generated HTML reports and email templates.
...
- **🐛 Bug Fixes**:
  ...
  - **Retry tabs not showing**: Fixed the retry tabs not showing in the report, when clicked on "Test Run Summary" tab.
...
- **🐛 Bug Fixes**:
  ...
  - **Fixed all kind of vulnerabilities**: Addresses security concerns by updating dependencies to their latest versions, removing deprecated packages, and following best practices for secure coding. _No Open Source Security Issue_, _No Code Security Issue_, _No Configuration Issue_.
...
- **Severity Distribution Chart**: Added a new "Severity Distribution" chart with _lazy-loading_ support to the "Dashboard" tab, visualizing the breakdown of test results by severity level (Minor, Major, Critical) to highlight priority areas.
